  ...
  case 0:
    if(total == -1) {
      safe_snprintf(__FILE__, __LINE__, buf, bufLen,
		    ...
		    "</TR>\n</TABLE></TD>"
		    "<TD ALIGN=RIGHT WIDTH=\"10%%\" BGCOLOR=\"%s\">%s 100&nbsp;%%</TD></TR>\n</TABLE></TD></TR>\n\n",
  ...
  case 100:
    if(total == -1) {
      safe_snprintf(__FILE__, __LINE__, buf, bufLen, "\n<TR %s><TH "TH_BG" ALIGN=LEFT "DARK_BG">%s</TH>"
		    ...
		    "<TD ALIGN=CENTER WIDTH=\"100%%\" BGCOLOR=\"%s\">&nbsp;</TD>"
		    "</TR>\n</TABLE></TD>"
		    "<TD ALIGN=RIGHT WIDTH=\"10%%\" BGCOLOR=\"%s\">%s 0&nbsp;%%</TD></TR>\n</TABLE></TD></TR>\n\n",
		    getRowColor(), label,
  ...
  default:
    if(total == -1) {
      safe_snprintf(__FILE__, __LINE__, buf, bufLen,
		    ...
		    "<TD ALIGN=CENTER WIDTH=\"%d%%\" BGCOLOR=\"%s\">&nbsp;</TD>"
		    "</TR>\n</TABLE></TD>"
		    "<TD ALIGN=RIGHT WIDTH=\"10%%\" BGCOLOR=\"%s\">%s %.1f&nbsp;%%</TD></TR>\n</TABLE></TD></TR>\n\n",
		    getRowColor(), label,
...
void printHeader(int reportType, int revertOrder, u_int column,
  ...
  sendString("<CENTER><TABLE WIDTH=100%% BORDER=0 "TABLE_DEFAULTS">\n<TR><TD ALIGN=LEFT>");
...
int cmpFctn(const void *_a, const void *_b) {
  ...
  case SORT_DATA_PROTOS:
    ...
    case 13:
      ...
      break;
    case 14:
      a_ = (*a)->ipBytesRcvd.value+(*a)->ipBytesSent.value, b_ = (*b)->ipBytesRcvd.value+(*b)->ipBytesSent.value;
      break;
    case 15:
      a_ = (*a)->nonIPTraffic == NULL ? 0 : (*a)->nonIPTraffic->stpSent.value+(*a)->nonIPTraffic->stpRcvd.value;
      b_ = (*b)->nonIPTraffic == NULL ? 0 : (*b)->nonIPTraffic->stpSent.value+(*b)->nonIPTraffic->stpRcvd.value;
...
int cmpMulticastFctn(const void *_a, const void *_b) {
  ...
  switch(myGlobals.columnSort) {
  case 2:
    if((*a)->pktsMulticastSent.value < (*b)->pktsMulticastSent.value)
      return(1);
    else if ((*a)->pktsMulticastSent.value > (*b)->pktsMulticastSent.value)
      return(-1);
    else
      return(0);
    break; /* NOTREACHED */
  case 3:
    ...
    break; /* NOTREACHED */
  case 4:
    if((*a)->pktsMulticastRcvd.value < (*b)->pktsMulticastRcvd.value)
      return(1);
    else if ((*a)->pktsMulticastRcvd.value > (*b)->pktsMulticastRcvd.value)
      return(-1);
    else
      return(0);
    break; /* NOTREACHED */
...
void printHostTrafficStats(HostTraffic *el, int actualDeviceId) {
  ...
  totalSent += el->icmpSent.value+el->icmp6Sent.value+el->ipBytesSent.value;
  ...
  totalRcvd += el->icmpRcvd.value+el->icmp6Rcvd.value+el->ipBytesRcvd.value;
  ...
  */
  ...
  printTableDoubleEntry(buf, sizeof(buf), "IPv6", CONST_COLOR_1, (float)el->ipBytesSent.value/1024,
			100*((float)SD(el->ipBytesSent.value, totalSent)),
			(float)el->ipBytesRcvd.value/1024,
			100*((float)SD(el->ipBytesRcvd.value, totalRcvd)));
...

  {
    totalSent = el->tcpSentLoc.value+el->tcpSentRem.value+
      ...
      el->icmpSent.value+el->icmp6Sent.value+el->ipBytesSent.value+
      el->greSent.value+el->ipsecSent.value;

    totalRcvd = el->tcpRcvdLoc.value+el->tcpRcvdFromRem.value+
      el->udpRcvdLoc.value+el->udpRcvdFromRem.value+
      el->icmpRcvd.value+el->icmp6Rcvd.value+el->ipBytesRcvd.value+
	...
	sendString("<TD "TD_BG" ALIGN=RIGHT COLSPAN=2 WIDTH=250>&nbsp;</TD>");
      ...
      for(i=0; i<myGlobals.l7.numSupportedProtocols; i++)
	sent += el->l7.traffic[i].bytesSent, rcvd += el->l7.traffic[i].bytesRcvd;

      for(i=0; i<myGlobals.l7.numSupportedProtocols; i++) {
	if(el->l7.traffic[i].bytesSent || el->l7.traffic[i].bytesRcvd) {
	  if(!headerAlreadySent) {
	    sendString("<TR "DARK_BG"><TH "TH_BG" WIDTH=100>L7 Protocol</TH>"
		       "<TH "TH_BG" WIDTH=200 COLSPAN=2>Data&nbsp;Sent</TH>"
		       "<TH "TH_BG" WIDTH=200 COLSPAN=2>Data&nbsp;Rcvd</TH></TR>\n\n");
	  ...
	  printTableDoubleEntry(buf, sizeof(buf),
				...
				100*((float)SD(el->l7.traffic[i].bytesRcvd, rcvd)));
	}
      }
...
void printIcmpv6Stats(HostTraffic *el) {
  ...
  printSectionTitle("ICMPv6 Traffic\n");
...
HostTraffic* quickHostLink(HostSerialIndex theSerialIdx, int deviceId, HostTraffic *el) {
  HostSerial theSerial;

  if(cmpSerial(&theSerialIdx, &myGlobals.broadcastEntry->serialHostIndex)) {
    memcpy(el, myGlobals.broadcastEntry, sizeof(HostTraffic));
    return(el);
  } else if(cmpSerial(&theSerialIdx, &myGlobals.otherHostEntry->serialHostIndex)) {
    memcpy(el, myGlobals.otherHostEntry, sizeof(HostTraffic));
    return(0);
  } else
    getHostSerialFromId(theSerialIdx, &theSerial);

  memset(el, 0, sizeof(HostTraffic));
  copySerial(&el->serialHostIndex, &theSerialIdx);
...
void printHostContactedPeers(HostTraffic *el, int actualDeviceId) {
  ...
  if(el->pktsSent.value > 0) {
	...
	theHost = getNextHost(actualDeviceId, theHost)) {
      if((theHost->l2Host == el->l2Host) && (theHost->hostIpAddress.hostFamily == el->hostIpAddress.hostFamily)) {
	...
	if(val > 0) {
	  ...
	  if(!header_sent) {
	    header_sent = 1;
	    sendString("<TD "TD_BG" VALIGN=TOP>"
		       "<div style=\"height:120px;width:500px;overflow-x:hidden;overflow-y:scroll;\">"
		       TABLE_ON"<TABLE BORDER=1 "TABLE_DEFAULTS" WIDTH=480>"
...
void printHostDetailedInfo(HostTraffic *el, int actualDeviceId) {
  ...
  if(el->hostNumIpAddress[0] != '\0') {
    ...
    safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf),
		  ...
		  "HREF=\"/"CONST_PURGE_HOST"?key=%u\">Purge Asset</A> <IMG SRC=/lock.png> ]",
		  el->serialHostIndex);
    sendString(buf);
    ...
    if(el->l2Host) {
      ...
      for(theHost = getFirstHost(actualDeviceId); theHost != NULL; theHost = getNextHost(actualDeviceId, theHost)) {
	if((theHost != el)
	   && (!theHost->l2Host)
	   && (memcmp(theHost->lastEthAddress, el->ethAddress, LEN_ETHERNET_ADDRESS) == 0)) {

	  if(!header_sent) {
	    safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf),
			  "<TR "TR_ON" %s><TH "TH_BG" ALIGN=LEFT "DARK_BG">%s</TH><TD "TD_BG" ALIGN=RIGHT>"
			  "<div style=\"height:120px;width:400px;overflow-x:hidden;overflow-y:auto;\">",
			  getRowColor(), "IP Addresses Bound to this MAC");

	    sendString(buf);
	    header_sent = 1;
	  }

	  sendString(makeHostLink(theHost, 0, 0, 0, buf, sizeof(buf)));
	  sendString("</br>\n");
	}
      } /* for */
  ...
  safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf), "<TR "TR_ON" %s><TH "TH_BG" ALIGN=LEFT "DARK_BG">%s"
	      ...
	      formatPkts(el->pktsSent.value, formatBuf1, sizeof(formatBuf1)),
	      formatPkts(el->pktsDuplicatedAckSent.value, formatBuf2, sizeof(formatBuf2)),
	      (int)(((float)el->pktsDuplicatedAckSent.value*100)/(float)(el->pktsSent.value+1))
	      );
  sendString(buf);
  ...
  safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf),
		"<TR "TR_ON" %s><TH "TH_BG" ALIGN=LEFT "DARK_BG">%s</TH><TD "TD_BG" ALIGN=RIGHT>"
	      ...
	      formatPkts(el->pktsBroadcastSent.value, formatBuf, sizeof(formatBuf)));
  sendString(buf);
  ...
  if((el->pktsMulticastSent.value > 0) || (el->pktsMulticastRcvd.value > 0)) {
    ...
    if(el->pktsMulticastSent.value > 0) {
      safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf), "Sent&nbsp;%s/%s&nbsp;Pkts&nbsp;",
		  ...
		  formatPkts(el->pktsMulticastSent.value, formatBuf1, sizeof(formatBuf1)));
      sendString(buf);
    }

    if(el->pktsMulticastRcvd.value > 0) {
      safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf), "%sRcvd&nbsp;%s/%s&nbsp;Pkts",
		    (el->pktsMulticastSent.value > 0) ? " - " : "",
		    formatBytes(el->bytesMulticastRcvd.value, 1, formatBuf, sizeof(formatBuf)),
		    formatPkts(el->pktsMulticastRcvd.value, formatBuf1, sizeof(formatBuf1)));
      sendString(buf);
    }
  ...
  safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf), "<TR "TR_ON" %s><TH "TH_BG" ALIGN=LEFT "DARK_BG">%s"
	      ...
	      formatPkts(el->pktsRcvd.value, formatBuf1, sizeof(formatBuf1)),
	      formatPkts(el->pktsDuplicatedAckRcvd.value, formatBuf2, sizeof(formatBuf2)),
	      (int)((float)(el->pktsDuplicatedAckRcvd.value*100)/(float)(el->pktsRcvd.value+1)));
  sendString(buf);
  ...
  total = el->pktsSent.value+el->pktsRcvd.value;
  if(total > 0) {
    percentage = ((float)el->pktsSent.value*100)/((float)total);
